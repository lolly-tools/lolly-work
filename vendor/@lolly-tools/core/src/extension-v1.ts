 ...
 *
 * The typed DOOR + FURNITURE-SPEC: the host-v1 analog for CHROME slots. Lolly's
 * founding thesis is "tools are data, not bundled code — a contract + runtime
 * hydration + multiple channels, so new tools ship without an app rebuild". This
 * is that same idea applied to chrome/governance surfaces instead of the tool
 * canvas: core defines named SLOTS with a typed contract; components are HYDRATED
 * into them at runtime; two supply channels (a control plane + the community) fill
 * the same doors; an empty door renders nothing.
 *
 ...
 * one data constant only — importing it pulls in no furniture and no DOM.
 *
...
export type ExtensionSlotId =
  | 'cost-authoring'; // the first door (see SLOT_REGISTRY). More added additively.
...
/**
 * One enumerable slot descriptor. SLOT_REGISTRY lists every door so a control
 * plane or community deployer can DISCOVER what exists to fill — the chrome analog
 * of the tool catalog index. Pure data: no furniture, no DOM.
 */
export interface SlotManifest {
  readonly id: ExtensionSlotId;
  readonly cardinality: SlotCardinality;
  /** Dev-facing purpose of the door (not user copy). */
...
/**
 * The enumerable catalog of doors. Data only — no furniture, no DOM. Both channels
 * read it to know what slots exist; the shell's registry validates against it.
 */
...
export interface SlotHost<Ctx = unknown, El = unknown> {
   ...
   *  reach outside it (the door frame). Empty when the extension mounts. */
  readonly el: El;
  /** Which slot this is (redundant with Extension.slot; handy when one component
   *  serves several doors). */
  ...
  /** Localise a UI string (the shell's i18n `t`) — furniture stays translatable
   *  without importing the shell's i18n module. */
...
/**
 * What a hydrated component PROVIDES — the furniture. As small as a tool's
 * contract: an id, which door, a mount lifecycle.
 */
export interface Extension<Ctx = unknown, El = unknown> {
  ...
  readonly id: string;
  /** The door this fills. Must be a member of SLOT_REGISTRY. */
  readonly slot: ExtensionSlotId;
