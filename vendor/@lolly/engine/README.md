...
**172 modules** under `engine/src/` (excluding the `index.ts` barrel): 155 re-exported from `index.ts`, 139 with a dedicated `tests/*.test.ts`, 25 covered indirectly, 8 with no coverage under `tests/`, 19 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `docx-read.ts` | 997 | docx-read.ts: PARSE an unzipped .docx part map into `doc-model.ts` blocks. | yes | `tests/docx-read.test.ts` | yes |
| `docx.ts` | 795 | DOCX (Word / WordprocessingML OOXML) builder. | yes | `tests/docx.test.ts` | – |
...
| `pptx-read.ts` | 1241 | pptx-read.ts: PARSE an unzipped .pptx part map into a read-model. | yes | `tests/pptx-read.test.ts` | yes |
...
| `runtime.ts` | 1517 | Runtime - orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
