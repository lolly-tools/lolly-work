...
**186 modules** under `engine/src/` (excluding the `index.ts` barrel): 167 re-exported from `index.ts`, 149 with a dedicated `tests/*.test.ts`, 27 covered indirectly, 10 with no coverage under `tests/`, 19 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `chart-spec.ts` | 272 | Pure ChartSpecV1 helpers. | yes | `tests/chart-spec.test.ts` | – |
...
| `url-mode.ts` | 994 | URL mode. | yes | indirect | – |
