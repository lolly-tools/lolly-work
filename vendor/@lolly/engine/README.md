...
**194 modules** under `engine/src/` (excluding the `index.ts` barrel): 174 re-exported from `index.ts`, 154 with a dedicated `tests/*.test.ts`, 28 covered indirectly, 12 with no coverage under `tests/`, 28 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `runtime.ts` | 1640 | Runtime - orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
