 ...
 *
 * Tools needing real logic use hooks.js. It is a sandboxed escape hatch where the
...
// arrow: swap a leading direction marker for its arrow glyph. This is the single-line
...
// (relative / scheme-relative / fragment / query). Everything else, such as javascript:,
// vbscript:, or file:, is dropped, and the caller renders the plain text instead.
...
// Output is raw HTML (SafeString), so every interpolated value is escaped. This is the
 ...
 * <img src="{{asset id}}">) are the overwhelmingly common way a color or asset
 * input actually reaches the page. These get `data-canvas-input="id"` written
 ...
 * one it mentions (positionally). That is enough for the shell's mapping. Block
...
export function annotateTemplate(source: string, inputIds: string[]): string {
  ...
  // (positionally). That is enough for the shell's click-to-focus mapping.
