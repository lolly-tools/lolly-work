 ...
 * compose a `ZzfxSong` deterministically, and the same ref is a 20-second bed
 ...
 * a different seed, which would let a round-trip through `formatZzfxmRef` rewrite
...
export interface ZzfxmRef {
  ...
  /** The raw third segment, recognised or not — for the "ignored style" warning. */
  rawStyle?: string;
}
 ...
 * and then folded by `>>> 0` into `7` and `0` — so `format(parse(x)) !== x`, and a
 * regenerate action that round-trips a ref would quietly rewrite a shared link's
 ...
 *
 * Null is the answer for every ordinary url, which is what lets callers try this
 * first without risk. Note the asymmetry with a MALFORMED `zzfxm:` — a caller
 * distinguishes the two with `isZzfxmRef`, so `zzfxm:abc` can warn rather than
