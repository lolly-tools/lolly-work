// SPDX-License-Identifier: MPL-2.0
/**
 * Colour-vision-deficiency (CVD) simulation - Machado, Oliveira & Fernandes (2009).
 *
 *   Gustavo M. Machado, Manuel M. Oliveira, Leandro A. F. Fernandes,
 *   "A Physiologically-based Model for Simulation of Color Vision Deficiency",
 *   IEEE Transactions on Visualization and Computer Graphics 15(6), 2009,
 *   pp. 1291–1298.  Matrices from the authors' supplementary table:
 *   https://www.inf.ufrgs.br/~oliveira/pubs_files/CVD_Simulation/CVD_Simulation.html
 *
 * The paper models each of the three anomalous-trichromacy axes (protanomaly
 * = protan, deuteranomaly = deutan, tritanomaly = tritan) as a single 3x3
 * matrix per severity step, tabulated at severity 0.0 .. 1.0 in increments of
 * 0.1 (0 = normal vision, 1 = full dichromacy: protanopia / deuteranopia /
 * tritanopia). We golden-pin the published matrices EXACTLY and linearly
 * interpolate the nine entries between adjacent steps for in-between severities.
 *
 * SPACE / CONVENTION: the Machado matrices are applied as a LINEAR OPERATION on
 * the GAMMA-ENCODED sRGB channel values (the "linear-ish operation on sRGB"
 * convention: a matrix multiply in the sRGB-encoded [0,1] space, NOT in linear
 * light). This is the convention of the authors' own reference implementation
 * and of the de-facto standard libraries built from these tables, so our numbers
 * match theirs. We do NOT sRGB-decode before the multiply. Channels are clamped
 * to [0,1] after the multiply (the matrices can push out of gamut).
 *
 * The severity-0 matrix is the identity, so simulateCvd(rgb, type, 0) is exactly
 * rgb (bit-for-bit), for every type.
 *
 * toGrayscale() is a separate Rec.709 luma reduction (weights 0.2126 / 0.7152 /
 * 0.0722) applied to the same gamma-encoded sRGB values.
 *
 * Pure, DOM-free. RGB triples are 0..255 (integers on output, rounded).
 */

import { parseHex } from './brand-derive.ts';

/** The three colour-vision-deficiency axes the Machado model covers. */
export type CvdType = 'protan' | 'deutan' | 'tritan';

/** An RGB triple, channels 0..255. */
export type Rgb = [number, number, number];

type Mat3 = readonly [number, number, number, number, number, number, number, number, number];

// ─── Golden-pinned Machado (2009) matrices ───────────────────────────────────
// Row-major 3x3, one per severity step 0.0 .. 1.0 (index 0 .. 10). Each matrix
// maps a gamma-encoded sRGB column vector [r,g,b]^T to the simulated triple.
// Values are copied verbatim from the authors' supplementary table; do NOT
// "tidy" them - they are the published reference and are pinned exactly by the
// test suite.

// Protanomaly → protanopia.
const PROTAN: readonly Mat3[] = [
  // 0.0 - identity (normal vision)
  [1, 0, 0, 0, 1, 0, 0, 0, 1],
  // 0.1
  [0.856167, 0.182038, -0.038205, 0.029342, 0.955115, 0.015544, -0.00288, -0.001563, 1.004443],
  // 0.2
  [0.734766, 0.334872, -0.069637, 0.05184, 0.919198, 0.028963, -0.004928, -0.004209, 1.009137],
  // 0.3
  [0.630323, 0.465641, -0.095964, 0.069181, 0.890046, 0.040773, -0.006308, -0.007724, 1.014032],
  // 0.4
  [0.539009, 0.579343, -0.118352, 0.082546, 0.866121, 0.051332, -0.007136, -0.011959, 1.019095],
  // 0.5
  [0.458064, 0.679578, -0.137642, 0.092785, 0.846313, 0.060902, -0.007494, -0.016807, 1.024301],
  // 0.6
  [0.38545, 0.769005, -0.154455, 0.100526, 0.829802, 0.069673, -0.007442, -0.02219, 1.029632],
  // 0.7
  [0.319627, 0.849633, -0.169261, 0.106241, 0.815969, 0.07779, -0.007025, -0.028051, 1.035076],
  // 0.8
  [0.259411, 0.923008, -0.18242, 0.110296, 0.80434, 0.085364, -0.006276, -0.034346, 1.040622],
  // 0.9
  [0.203876, 0.990338, -0.194214, 0.112975, 0.794542, 0.092483, -0.005222, -0.041043, 1.046265],
  // 1.0 - protanopia
  [0.152286, 1.052583, -0.204868, 0.114503, 0.786281, 0.099216, -0.003882, -0.048116, 1.051998],
];

// Deuteranomaly → deuteranopia.
const DEUTAN: readonly Mat3[] = [
  // 0.0 - identity
  [1, 0, 0, 0, 1, 0, 0, 0, 1],
  // 0.1
  [0.866435, 0.177704, -0.044139, 0.049567, 0.939063, 0.01137, -0.003453, 0.007233, 0.99622],
  // 0.2
  [0.760729, 0.319078, -0.079807, 0.090568, 0.889315, 0.020117, -0.006027, 0.013325, 0.992702],
  // 0.3
  [0.675425, 0.43385, -0.109275, 0.125303, 0.847755, 0.026942, -0.00795, 0.018572, 0.989378],
  // 0.4
  [0.605511, 0.52856, -0.134071, 0.155318, 0.812366, 0.032316, -0.009376, 0.023176, 0.9862],
  // 0.5
  [0.547494, 0.607765, -0.155259, 0.181692, 0.781742, 0.036566, -0.01041, 0.027275, 0.983136],
  // 0.6
  [0.498864, 0.674741, -0.173604, 0.205199, 0.754872, 0.039929, -0.011131, 0.030969, 0.980162],
  // 0.7
  [0.457771, 0.731899, -0.18967, 0.226409, 0.731012, 0.042579, -0.011595, 0.034333, 0.977261],
  // 0.8
  [0.422823, 0.781057, -0.203881, 0.245752, 0.709602, 0.044646, -0.011843, 0.037423, 0.974421],
  // 0.9
  [0.392952, 0.82361, -0.216562, 0.263559, 0.69021, 0.046232, -0.01191, 0.040281, 0.97163],
  // 1.0 - deuteranopia
  [0.367322, 0.860646, -0.227968, 0.280085, 0.672501, 0.047413, -0.01182, 0.04294, 0.968881],
];

// Tritanomaly → tritanopia.
const TRITAN: readonly Mat3[] = [
  // 0.0 - identity
  [1, 0, 0, 0, 1, 0, 0, 0, 1],
  // 0.1
  [0.92667, 0.092514, -0.019184, 0.021191, 0.964503, 0.014306, 0.008437, 0.054813, 0.93675],
  // 0.2
  [0.89572, 0.13333, -0.02905, 0.029997, 0.9454, 0.024603, 0.013027, 0.104707, 0.882266],
  // 0.3
  [0.905871, 0.127791, -0.033662, 0.026856, 0.941251, 0.031893, 0.01341, 0.148296, 0.838294],
  // 0.4
  [0.948035, 0.08949, -0.037526, 0.014364, 0.946792, 0.038844, 0.010853, 0.193991, 0.795156],
  // 0.5
  [1.017277, 0.027029, -0.044306, -0.006113, 0.958479, 0.047634, 0.006379, 0.248708, 0.744913],
  // 0.6
  [1.104996, -0.046633, -0.058363, -0.032137, 0.971635, 0.060503, 0.001336, 0.317922, 0.680742],
  // 0.7
  [1.193214, -0.109812, -0.083402, -0.058496, 0.97941, 0.079086, -0.002346, 0.403492, 0.598854],
  // 0.8
  [1.257728, -0.139648, -0.118081, -0.078003, 0.975409, 0.102594, -0.003316, 0.501214, 0.502102],
  // 0.9
  [1.278864, -0.125333, -0.153531, -0.084748, 0.957674, 0.127074, -0.000989, 0.601151, 0.399838],
  // 1.0 - tritanopia
  [1.255528, -0.076749, -0.178779, -0.078411, 0.930809, 0.147602, 0.004733, 0.691367, 0.3039],
];

const TABLES: Record<CvdType, readonly Mat3[]> = {
  protan: PROTAN,
  deutan: DEUTAN,
  tritan: TRITAN,
};

// Rec.709 / sRGB luma coefficients (BT.709), applied to gamma-encoded sRGB.
const REC709 = { r: 0.2126, g: 0.7152, b: 0.0722 } as const;

const clamp01 = (n: number): number => (n < 0 ? 0 : n > 1 ? 1 : n);
const round255 = (n: number): number => Math.round(clamp01(n) * 255);

/**
 * The severity-graded Machado matrix for `type`. `severity` is clamped to
 * [0,1]; between tabulated steps the nine entries are linearly interpolated.
 */
function cvdMatrix(type: CvdType, severity: number): Mat3 {
  const table = TABLES[type];
  const s = severity < 0 ? 0 : severity > 1 ? 1 : severity;
  const scaled = s * 10; // step space: 0 .. 10
  const lo = Math.floor(scaled);
  if (lo >= 10) return table[10]!;
  const hi = lo + 1;
  const f = scaled - lo;
  if (f === 0) return table[lo]!;
  const a = table[lo]!;
  const b = table[hi]!;
  const out = new Array(9) as number[];
  for (let i = 0; i < 9; i++) out[i] = a[i]! + (b[i]! - a[i]!) * f;
  return out as unknown as Mat3;
}

/**
 * Simulate how an RGB colour appears to a viewer with the given colour-vision
 * deficiency at the given severity (0 = normal vision, 1 = full dichromacy).
 * Channels are 0..255; the returned triple is rounded integers. Severity 0 is
 * an exact identity.
 */
export function simulateCvd(rgb: Rgb, type: CvdType, severity: number): Rgb {
  const m = cvdMatrix(type, severity);
  // Gamma-encoded sRGB in [0,1] (see module header: no linearisation).
  const r = rgb[0] / 255;
  const g = rgb[1] / 255;
  const b = rgb[2] / 255;
  return [
    round255(m[0] * r + m[1] * g + m[2] * b),
    round255(m[3] * r + m[4] * g + m[5] * b),
    round255(m[6] * r + m[7] * g + m[8] * b),
  ];
}

/**
 * Rec.709 luminance greyscale of an RGB colour: a neutral triple whose value is
 * the BT.709 luma (0.2126 R + 0.7152 G + 0.0722 B) of the gamma-encoded input.
 */
export function toGrayscale(rgb: Rgb): Rgb {
  const y = Math.round(clamp01((REC709.r * rgb[0] + REC709.g * rgb[1] + REC709.b * rgb[2]) / 255) * 255);
  return [y, y, y];
}

const toHex = (rgb: Rgb): string =>
  '#' + rgb.map((c) => Math.round(c).toString(16).padStart(2, '0')).join('');

/**
 * Hex-in / hex-out convenience wrapper for {@link simulateCvd}. Returns null for
 * an unparseable hex string; alpha (if present) is dropped. Output is `#rrggbb`.
 */
export function simulateCvdHex(hex: string, type: CvdType, severity: number): string | null {
  const rgba = parseHex(hex);
  if (!rgba) return null;
  return toHex(simulateCvd([rgba[0], rgba[1], rgba[2]], type, severity));
}

/**
 * Hex-in / hex-out convenience wrapper for {@link toGrayscale}. Returns null for
 * an unparseable hex string. Output is `#rrggbb`.
 */
export function toGrayscaleHex(hex: string): string | null {
  const rgba = parseHex(hex);
  if (!rgba) return null;
  return toHex(toGrayscale([rgba[0], rgba[1], rgba[2]]));
}
