...
The install runs `npm ci` against the npm registry to fetch the server's runtime dependencies (about 300 MB), and downloads a prebuilt Lolly web app (a few hundred MB). Allow a few minutes on a small machine. The on-device machine-learning models the tools use are fetched by the browser on first use from `https://lolli.li`, the project's release host, not by the server.
