...
// Real work HTTP/auth/WebSocket services; Vite supplies only the development shell.
...
export async function createWorkBrowserFixture(ossDir: string, viteOrigin: string) {
  const pack = await mkdtemp(join(tmpdir(), 'lw-design-browser-'));
  ...
  const store = createMemoryStore(), secrets = { session: 'browser-session-test', link: 'browser-link-test' };
